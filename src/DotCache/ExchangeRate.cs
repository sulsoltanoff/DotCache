...
public struct ExchangeRate : IEquatable<ExchangeRate>
{
    ...
    private ExchangeRate(Currency baseCurrency, Currency quoteCurrency, decimal rate)
        : this()
    {
    ...
    private ExchangeRate(Currency baseCurrency, Currency quoteCurrency, double rate, int numberOfDecimals = 6)
    ...
    public static bool TryParse(string rate, out ExchangeRate result)
    {
        ...
        {
            if (ex is not FormatException && ex is not OverflowException && ex is not ArgumentException)
                throw;
