...
{
    /// <summary>Converts this <see cref="ValueObject.Money"/> instance to its equivalent <see cref="string"/> representation.</summary>
    /// <returns>A string that represents this <see cref="ValueObject.Money"/> instance.</returns>
    /// <remarks>
    /// Converting will use the <see cref="NumberFormatInfo"/> object for the current culture if this has the same
    /// ISOCurrencySymbol, otherwise the <see cref="NumberFormatInfo"/> from the <see cref="ValueObject.Currency"/> will be used.
    /// </remarks>
    public override string ToString() => ConvertToString(null, null);

    /// <summary>Converts the <see cref="ValueObject.Money"/> value of this instance to its equivalent <see cref="string"/> representation
    /// using the specified format.</summary>
    /// <param name="format">A numeric format string.</param>
    /// <returns>The string representation of this <see cref="ValueObject.Money"/> instance as specified by the format.</returns>
    public string ToString(string format)
    {
        return ConvertToString(format, null);
    }

    /// <summary>Converts this <see cref="ValueObject.Money"/> instance to its equivalent <see cref="string"/> representation using the
    /// specified culture-specific format information.</summary>
    /// <param name="formatProvider">An <see cref="IFormatProvider"/> that supplies culture-specific formatting information.</param>
    /// <returns>The string representation of this <see cref="ValueObject.Money"/> instance as specified by formatProvider.</returns>
    ...
    /// <param name="format">A numeric format string.</param>
    /// <param name="formatProvider">An <see cref="IFormatProvider"/> that supplies culture-specific formatting information.</param>
    /// <returns>The string representation of this <see cref="ValueObject.Money"/> instance as specified by the format and formatProvider.</returns>
    ...
    private string ConvertToString(string? format, IFormatProvider formatProvider)
    {
        ...
        if (!string.IsNullOrWhiteSpace(format) && format.StartsWith("I", StringComparison.Ordinal) && format.Length is >= 1 and <= 2)
        {
